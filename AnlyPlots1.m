function [imO,imO2d,imHistZ,imHistR,imHistX,imRg,imRg2d,imAR,imAR2d] = AnlyPlots1(Simul,Rtab,Anly)
...
if Nc1
	rcm = Nm/Nc1;
	Zc = rcm * sum(Simul.HistZ(HistSet,:,2))./diff(ZEs)/HistLength;
...
if Nc1
	rcm = Nm/Nc1;
	Rc = rcm * sum(Simul.HistR(HistSet,:,2))./diff(REs)./R/HistLength;
...
if Nc1
	rcm = Nm/Nc1;
	Xc = rcm * sum(Simul.HistX(HistSet,:,2))./diff(XEs)/HistLength;
